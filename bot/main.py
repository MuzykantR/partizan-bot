...
async def main():
    ...
    # Mode 1: Production Webhook with Secret Token validation
    if settings.bot_mode == "webhook" and settings.bot_token != "YOUR_TELEGRAM_BOT_TOKEN":
        try:
            ...
            if webhook_info.url != settings.webhook_url:
                ...
                logger.info(f"Telegram Webhook configured successfully: {settings.webhook_url}")
            else:
                logger.info(f"Telegram Webhook already active: {settings.webhook_url}")
        except Exception as e:
            logger.error(f"Failed to configure Telegram webhook: {e}")

        logger.info(f"Starting Partizan Bot & API in WEBHOOK mode on {settings.api_host}:{settings.api_port}...")
        await run_fastapi()

    # Mode 2: Local development Polling fallback
    else:
        logger.info("Starting Partizan Bot & API in POLLING fallback mode...")
        try:
